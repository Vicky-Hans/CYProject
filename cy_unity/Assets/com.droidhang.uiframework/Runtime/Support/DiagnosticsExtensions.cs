...
namespace DH.UIFramework
{
    internal static class DiagnosticsExtensions
    {
        ...
        static string ApplicationDataPath
        {
            get
            {
                applicationDataPath ??= Application.dataPath.Replace(Path.DirectorySeparatorChar, '/');
        ...
        static bool IgnoreLine(MethodBase methodInfo)
        {
            ...
            else if (declareType.StartsWith("System.Runtime.CompilerServices",StringComparison.Ordinal))
            {
                return true;
            }
            else if (declareType.StartsWith("Cysharp.Threading.Tasks.CompilerServices",StringComparison.Ordinal))
            {
                return true;
            }
            ...
            else if (declareType.StartsWith("System.Threading.Tasks.Task",StringComparison.Ordinal))
            {
                return true;
            }
            else if (declareType.StartsWith("Cysharp.Threading.Tasks.UniTaskCompletionSourceCore",StringComparison.Ordinal))
            {
                return true;
            }
            else if (declareType.StartsWith("Cysharp.Threading.Tasks.AwaiterActions",StringComparison.Ordinal))
            {
                return true;
            }
            else if (declareType.StartsWith("System.Reflection",StringComparison.Ordinal))
            {
                return true;
            }
            else if (declareType.StartsWith("System.RuntimeType",StringComparison.Ordinal))
            {
                return true;
            }
        ...
        static string AppendHyperLink(string path, string line)
        {
            ...
            {
                if (fi.FullName.Contains("DataBindingGenerator"))
                {
                    return path;
                }
                
                var fname = fi.FullName.Replace(Path.DirectorySeparatorChar, '/').Replace(ApplicationDataPath, "");
