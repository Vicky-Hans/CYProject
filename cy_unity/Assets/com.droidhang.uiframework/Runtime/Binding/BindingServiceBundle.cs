...
namespace DH.UIFramework
{
    public class BindingServiceBundle : AbstractServiceBundle
    {
        ...
        protected override void OnStart(IServiceContainer container)
        {
            ...
            sourceFactory.Register(new CompiledExpressionSourceProxyFactory(sourceFactory), 0);
            //sourceFactory.Register(new LiteralSourceProxyFactory(), 1);
            //sourceFactory.Register(new ExpressionSourceProxyFactory(sourceFactory, expressionPathFinder), 2);
            sourceFactory.Register(objectSourceProxyFactory, 3);

            TargetProxyFactory targetFactory = new TargetProxyFactory();
            targetFactory.Register(new CompiledTargetProxyFactory(), 0);
