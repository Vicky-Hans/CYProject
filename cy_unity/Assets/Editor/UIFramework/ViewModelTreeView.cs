...
{
    public class ViewModelTrackerViewItem : TreeViewItem
    {
        ...
        public ViewModelTrackerViewItem(int id) : base(id)
        {

        }
    }

    public class ViewModelTrackerTreeView : TreeView
    {
        const string sortedColumnIndexStateKey = "ViewModelTrackerTreeView_sortedColumnIndex";

        public IReadOnlyList<TreeViewItem> CurrentBindingItems;

        public ViewModelTrackerTreeView()
            : this(new TreeViewState(), new MultiColumnHeader(new MultiColumnHeaderState(new[]
            {
                new MultiColumnHeaderState.Column() { headerContent = new GUIContent("ViewModelType"), width = 20},
        ...
        ViewModelTrackerTreeView(TreeViewState state, MultiColumnHeader header)
            : base(state, header)
        {
            rowHeight = 20;
        ...
        private void Header_sortingChanged(MultiColumnHeader multiColumnHeader)
        {
            ...
            var items = rootItem.children.Cast<ViewModelTrackerViewItem>();

            IOrderedEnumerable<ViewModelTrackerViewItem> orderedEnumerable;
            switch (index)
            {
                case 0:
        ...
        protected override TreeViewItem BuildRoot()
        {
            ...
            ViewModelTracker.ForEachActiveTask((trackingId, awaiterType, created, stackTrace) =>
            {
                children.Add(new ViewModelTrackerViewItem(trackingId) { TaskType = awaiterType, Elapsed = (DateTime.UtcNow - created).TotalSeconds.ToString("00.00"), Position = stackTrace });
            });
        ...
        {
            var item = args.item as ViewModelTrackerViewItem;
