...
namespace RVO
{
    ...
    public class Simulator
    {
        ...
        private class Worker
        {
            ...
            internal void UpdateIndex(int start, int end)
            {
                start_ = start;
                end_ = end;
            }
        ...
        public static readonly ProfilerMarker RvoSimulatorProfilerMarker = new ProfilerMarker("RvoSimulator");
        public static readonly ProfilerMarker RvoBuildAgentTreeProfilerMarker = new ProfilerMarker("RvoSimulatorBuildAgentTree");
        public static readonly ProfilerMarker RvoPrepareProfilerMarker = new ProfilerMarker("RvoSimulatorPrepare");
        public static readonly ProfilerMarker RvoSimulatorUpdateProfilerMarker = new ProfilerMarker("RvoSimulatorUpdate");

        internal IList<Agent> agents_;
        internal IList<Obstacle> obstacles_;
        internal KdTree kdTree_;
        internal float timeStep_;
        internal bool agentCountChanged;
         ...
         * <param name="agentUnit"></param>
         */
        public int addAgent(Vector2 position,float radius,IRvoAgentUnit agentUnit)
        {
            ...
            agent.maxSpeed_ = agentUnit.Speed;
         ...
         */
        public float doStepSync()
        {
            RvoSimulatorProfilerMarker.Begin();
            RvoBuildAgentTreeProfilerMarker.Begin();
            kdTree_.buildAgentTree();
            RvoBuildAgentTreeProfilerMarker.End();

            RvoPrepareProfilerMarker.Begin();
            int agentCount = getNumAgents();
            for (int agentNo = 0; agentNo < agentCount; ++agentNo)
            {
                agents_[agentNo].computeNeighbors();
                agents_[agentNo].computeNewVelocity();
            }
            RvoPrepareProfilerMarker.End();

            RvoSimulatorUpdateProfilerMarker.Begin();
            for (int agentNo = 0; agentNo < agentCount; ++agentNo)
            {
                agents_[agentNo].update();
            }
            RvoSimulatorUpdateProfilerMarker.End();

            globalTime_ += timeStep_;
            RvoSimulatorProfilerMarker.End();
            return globalTime_;
        }
        ...
        public float doStep()
        {
            ...
            if (workers_ == null)
            {
                ...
                for (int block = 0; block < numWorkers_; ++block)
                {
                    ...
                    workers_[block] = new Worker(block * getNumAgents() / numWorkers_, (block + 1) * getNumAgents() / numWorkers_, doneEvents_[block]);
                }

                agentCountChanged = false;
            }
            ...
            if (agentCountChanged)
            {
                ...
                for (int block = 0; block < numWorkers_; ++block)
                {
                    workers_[block].UpdateIndex(block * agentCount /numWorkers_, (block + 1) * agentCount / numWorkers_);
                }
