...
namespace Pathfinding.DHRVO
{
	...
	public class Agent : IAgent {
		...
		public Agent (TSVector2 pos, FP elevationCoordinate) {
			AgentTimeHorizon = 2;
		...
		/// </summary>
		internal FP InsertAgentNeighbour (Agent agent, FP rangeSq) {
			// Check if this agent collides with the other agent
			if (this == agent || (agent.layer & collidesWith) == 0) return rangeSq;

			// 2D distance
			FP dist = (agent.position - position).sqrMagnitude;

			if (dist < rangeSq) {
				if (neighbours.Count < maxNeighbours) {
					neighbours.Add(null);
					neighbourDists.Add(FP.PositiveInfinity);
				}
		...
		static void DrawVO (TSVector2 circleCenter, FP radius, TSVector2 origin) {
			FP alpha = Mathf.Atan2((origin - circleCenter).y, (origin - circleCenter).x);
			FP gamma = radius/(origin-circleCenter).magnitude;
			FP delta = gamma <= 1.0f ? Mathf.Abs(Mathf.Acos(gamma)) : 0;

			Draw.Debug.CircleXZ(FromXZ(circleCenter), radius, Color.black, alpha-delta, alpha+delta);
			TSVector2 p1 = new TSVector2(Mathf.Cos(alpha-delta), Mathf.Sin(alpha-delta)) * radius;
			TSVector2 p2 = new TSVector2(Mathf.Cos(alpha+delta), Mathf.Sin(alpha+delta)) * radius;
		...
		internal struct VO {
			...
			public VO (TSVector2 center, TSVector2 offset, FP radius, FP inverseDt, FP inverseDeltaTime) {
				...
				if (center.magnitude < radius) {
					...
					FP alpha = Mathf.Atan2(-center.y, -center.x);

					FP delta = Mathf.Abs(Mathf.Acos(radius/center.magnitude));
					...
					// Point on circle
					line1 = new TSVector2(Mathf.Cos(alpha+delta), Mathf.Sin(alpha+delta));
					...
					// Point on circle
					line2 = new TSVector2(Mathf.Cos(alpha-delta), Mathf.Sin(alpha-delta));
					// Vector tangent to circle which is the correct line tangent
					// Note that this vector is normalized
			...
			public static VO SegmentObstacle (TSVector2 segmentStart, TSVector2 segmentEnd, TSVector2 offset, FP radius, FP inverseDt, FP inverseDeltaTime) {
				...
				var closestOnSegment = VectorMath.ClosestPointOnSegment(segmentStart, segmentEnd, TSVector2.zero);
				...
				if (closestOnSegment.magnitude <= radius) {
					...
					// The call to Max is just to prevent FPing point errors causing NaNs to appear
					var startSqrMagnitude = segmentStart.sqrMagnitude;
					var normal1 = -VectorMath.ComplexMultiply(segmentStart, new TSVector2(radius, Mathf.Sqrt(Mathf.Max(0, startSqrMagnitude - radius*radius)))) / startSqrMagnitude;
					var endSqrMagnitude = segmentEnd.sqrMagnitude;
					var normal2 = -VectorMath.ComplexMultiply(segmentEnd, new TSVector2(radius, -Mathf.Sqrt(Mathf.Max(0, endSqrMagnitude - radius*radius)))) / endSqrMagnitude;
					...
					if (det1 >= 0 && det2 >= 0) {
						...
						if (TSVector2.Dot(p - line1, dir1) > 0 && TSVector2.Dot(p - line2, dir2) < 0) {
							if (segment) {
								...
								if (det3 < radius) {
									var closestPointOnLine = (TSVector2)VectorMath.ClosestPointOnSegment(segmentStart, segmentEnd, p);
		...
		internal void CalculateVelocity (Pathfinding.DHRVO.Simulator.WorkerContext context) {
			if (manuallyControlled) {
				return;
			}
				...
				do {
					...
					if (dist >= -0.01f && dist < range) {
						...
						if (sqrDistToSegment < range*range && (simulator.movementPlane == MovementPlane.XY || (elevationCoordinate <= segmentY + vertex.height && elevationCoordinate+height >= segmentY))) {
							vos.Add(VO.SegmentObstacle(p2 - position, p1 - position, TSVector2.zero, radius * 0.01f, 1f / ObstacleTimeHorizon, 1f / simulator.DeltaTime));
						}
					}
		...
		void GenerateNeighbourAgentVOs (VOBuffer vos) {
			FP inverseAgentTimeHorizon = 1.0f/agentTimeHorizon;
			...
			for (int o = 0; o < neighbours.Count; o++) {
				...
				FP maxY = System.Math.Min(elevationCoordinate + height, other.elevationCoordinate + other.height);
				FP minY = System.Math.Max(elevationCoordinate, other.elevationCoordinate);
				...
				FP totalRadius = radius + other.radius;

				// Describes a circle on the border of the VO
				TSVector2 voBoundingOrigin = other.position - position;
				...
				TSVector2 otherOptimalVelocity = TSVector2.Lerp(other.currentVelocity, other.desiredVelocity, 2*avoidanceStrength - 1);
		...
		TSVector2 GradientDescent (VOBuffer vos, TSVector2 sampleAround1, TSVector2 sampleAround2) {
			...
			// for ( int i = 0; i < 3; i++ ) {
			//	TSVector2 p = desiredVelocity + new TSVector2(Mathf.Cos(Mathf.PI*2*(i/3.0f)), Mathf.Sin(Mathf.PI*2*(i/3.0f)));
		...
		TSVector2 Trace (VOBuffer vos, TSVector2 p, out FP score) {
			...
			FP stepSize = Mathf.Max(radius, 0.2f * desiredSpeed);

			FP bestScore = FP.PositiveInfinity;
			TSVector2 bestP = p;
			...
			for (int s = 0; s < MaxIterations; s++) {
				FP step = 1.0f - (s/(FP)MaxIterations);
