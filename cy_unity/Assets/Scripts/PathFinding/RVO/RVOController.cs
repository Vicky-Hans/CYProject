...
namespace Pathfinding.DHRVO
{
	...
	public class RVOController : MonoBehaviour {
		...
		public FP obstacleTimeHorizon = 2;
		...
		/// <summary>\details Deprecated:</summary>
		[System.Obsolete("This field is obsolete in version 4.0 and will not affect anything. Use the LegacyRVOController if you need the old behaviour")]
		public FP rotationSpeed { get { return 0; } set {} }

		/// <summary>\details Deprecated:</summary>
		[System.Obsolete("This field is obsolete in version 4.0 and will not affect anything. Use the LegacyRVOController if you need the old behaviour")]
		public FP maxSpeed { get { return 0; } set {} }
		...
		///
		/// See: <see cref="IAgent::ForceSetVelocity"/>
		...
		public TSVector CalculateMovementDelta (FP deltaTime) {
			if (rvoAgent == null) return TSVector.zero;
			return To3D(TSVector2.ClampMagnitude(rvoAgent.CalculatedTargetPoint - To2D(tr.position), rvoAgent.CalculatedSpeed * deltaTime), 0);
		}
		...
		/// <param name="deltaTime">How far to move [seconds].
		///      Usually set to Time.deltaTime.</param>
		public TSVector CalculateMovementDelta (TSVector position, FP deltaTime) {
			return To3D(TSVector2.ClampMagnitude(rvoAgent.CalculatedTargetPoint - To2D(position), rvoAgent.CalculatedSpeed * deltaTime), 0);
		}

		/// <summary>\copydoc Pathfinding::RVO::IAgent::SetCollisionNormal</summary>
		public void SetCollisionNormal (TSVector normal) {
		...
		public void ForceSetVelocity (TSVector velocity) {
		...
		/// </summary>
		public TSVector2 To2D (TSVector p) {
			FP dummy;
		...
		/// </summary>
		public TSVector2 To2D (TSVector p, out FP elevation) {
		...
		/// See: movementPlane
		/// </summary>
		public TSVector To3D (TSVector2 p, FP elevationCoordinate) {
			if (movementPlane == MovementPlane.XY) {
				return new TSVector(p.x, p.y, -elevationCoordinate);
			} else {
				return new TSVector(p.x, elevationCoordinate, p.y);
			}
		}
		...
		void OnEnable () {
			...
			if (RVOSimulator.active == null) {
				...
				if (rvoAgent != null) {
					...
					rvoAgent = simulator.AddAgent(TSVector2.zero, 0);
		...
		protected void UpdateAgentProperties () {
			...
			rvoAgent.Position = To2D(tr.position, out elevation);
		...
		public void SetTarget (TSVector pos, FP speed, FP maxSpeed) {
			if (simulator == null) return;
		...
		public void Teleport (TSVector pos) {
			tr.position = pos;
		}
		...
		void OnDrawGizmos () {
			...
			var scale = tr.localScale;
			if (movementPlane == MovementPlane.XY) {
				Draw.Gizmos.Cylinder(pos, TSVector.forward, 0, radius * scale.x, color);
			} else {
				Draw.Gizmos.Cylinder(pos + To3D(TSVector2.zero, center - height * 0.5f) * scale.y, To3D(TSVector2.zero, 1), height * scale.y, radius * scale.x, color);
			}
		}
	}
}