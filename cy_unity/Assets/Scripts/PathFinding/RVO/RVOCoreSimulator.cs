...
namespace Pathfinding.DHRVO {
	...
	public interface IAgent {
		...
		///
		/// Note that this is a TSVector2, not a TSVector since the RVO simulates everything internally in 2D. So if your agents move in the
		/// XZ plane you may have to convert it to a TSVector like this.
		///
		/// <code>
		/// TSVector position3D = new TSVector(agent.Position.x, agent.ElevationCoordinate, agent.Position.y);
		/// </code>
		/// </summary>
		...
		/// </summary>
		FP ElevationCoordinate { get; set; }
		...
		///
		/// Note: This is a TSVector2, not a TSVector as that is what the <see cref="SetTarget"/> method accepts.
		///
		/// See: RVOController.CalculateMovementDelta.
		/// </summary>
		TSVector2 CalculatedTargetPoint { get; }
		...
		///      Note that this is a TSVector2, not a TSVector since the system simulates everything internally in 2D. So if your agents move in the
		///      XZ plane you will have to supply it as a TSVector2 with (x,z) coordinates.</param>
		...
		void SetTarget(TSVector2 targetPoint, FP desiredSpeed, FP maxSpeed);
		...
		/// </summary>
		void SetCollisionNormal(TSVector2 normal);
		...
		/// </summary>
		void ForceSetVelocity(TSVector2 velocity);
	}
	...
	public class Simulator {
		...
		/// </summary>
		private FP desiredDeltaTime = 0.05f;
		...
		/// </summary>
		public FP DesiredDeltaTime { get { return desiredDeltaTime; } set { desiredDeltaTime = System.Math.Max(value, 0.0f); } }
		...
		/// </summary>
		public FP symmetryBreakingBias = 0.1f;
		...
		///
		/// Deprecated: Use AddAgent(TSVector2,FP) instead
		/// </summary>
		[System.Obsolete("Use AddAgent(TSVector2,FP) instead")]
		public IAgent AddAgent (TSVector position) {
			return AddAgent(new TSVector2(position.x, position.z), position.y);
		}
		...
		public IAgent AddAgent (TSVector2 position, FP elevationCoordinate) {
		...
		/// See: RemoveObstacle
		/// </summary>
		public ObstacleVertex AddObstacle (TSVector[] vertices, FP height, bool cycle = true) {
		...
		/// See: RemoveObstacle
		/// </summary>
		public ObstacleVertex AddObstacle (TSVector[] vertices, FP height, Matrix4x4 matrix, RVOLayer layer = RVOLayer.DefaultObstacle, bool cycle = true) {
		...
		/// See: RemoveObstacle
		/// </summary>
		public ObstacleVertex AddObstacle (TSVector a, TSVector b, FP height) {
			...
			first.dir = new TSVector2(b.x-a.x, b.z-a.z).normalized;
		...
		public void UpdateObstacle (ObstacleVertex obstacle, TSVector[] vertices, Matrix4x4 matrix) {
			if (vertices == null) throw new System.ArgumentNullException("Vertices must not be null");
			...
			do {
				if (vertex.next == null) {
					vertex.dir = TSVector2.zero;
				} else {
					TSVector dir = vertex.next.position - vertex.position;
					vertex.dir = new TSVector2(dir.x, dir.z).normalized;
				}
