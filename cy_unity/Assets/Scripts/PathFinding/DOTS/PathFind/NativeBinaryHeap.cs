...
namespace PathFinding.DOTS
{
    public struct NativeBinaryHeap : IDisposable
    {
        ...
        private NativeArray<Tuple> heap;
        ...
        public NativeBinaryHeap (int capacity) {
            ...
            heap = new NativeArray<Tuple>(capacity, Allocator.TempJob);
            numberOfItems = 0;
        }
        ...
        public void Clear () {
            ...
            {
	            var item = heap[i];
	            var pathNode = item.node;
	            pathNode.HeapIndex = NotInHeap;
	            item.node = pathNode;
                heap[i] = item; 
            }
#endif
		...
		void Expand () {
			...
			int newSize = math.max(heap.Length+4, math.min(65533, heap.Length*growthFactor));
			...
			if (newSize > (1<<16) - 2) {
				Debug.LogError("Binary Heap Size really large (>65534). A heap size this large is probably the cause of pathfinding running in an infinite loop. ");
			}

			var newHeap = new NativeArray<Tuple>(newSize, Allocator.TempJob);
			heap.CopyTo(newHeap);
#if ASTARDEBUG
		...
		public void Add (PathNode node) {
			if (node.MeshNode.V0 == 0 && node.MeshNode.V1 == 0 && node.MeshNode.V2 == 0)
			{
...
#if DECREASE_KEY
					var heapItem = heap[bubbleIndex];
					var itemNode = heapItem.node;
					itemNode.HeapIndex = (ushort)bubbleIndex;
					heapItem.node = itemNode;
					heap[bubbleIndex] = heapItem;
#endif
...
#if DECREASE_KEY
					var heapItem = heap[parent];
					var itemNode = heapItem.node;
					itemNode.HeapIndex = (ushort) parent;
					heapItem.node = itemNode;
					heap[parent] = heapItem;
#endif
				}
				else
				{
...
#if DECREASE_KEY
						heapItem = heap[parentIndex];
						itemNode =	heapItem.node;
						itemNode.HeapIndex = (ushort)parentIndex;
						heapItem.node = itemNode;
						heap[parentIndex] = heapItem;
#endif
