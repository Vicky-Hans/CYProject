...
namespace DH.Game
{
    public partial class PlayerSkill1400 : BaseSkill
    {
        ...
        public override async void Shoot(WeaponSkill weaponSkill)
        {
            ...
            for(var i=0; i<atkNum; i++)
            {
                ...
                var bullet = InstantiateObj(bulletPath1, targetPos, Quaternion.identity, fightingManager.fightPanelTrans);
                var bulletComp = bullet.GetComponent<PlayerBullet1400>();
                var dmgArgs = CreateDamage();
                dmgArgs.weaponSkill = weaponSkill;
                dmgArgs.weaponModelId = weaponModelId;
                bulletComp.TargetMonster = target;
                bulletComp.InitWithTarget(dmgArgs, GetConfigArgs, targetPos, targetPos, this);
