...
namespace DH.Game
{
    ...
    public class CharacterController2D : MonoBehaviour
    {
        ...
        [SerializeField] [Range(0.001f, 0.3f)] float _skinWidth = 0.02f;
        ...
        private void Update()
        {
            Debug.DrawRay(rayStart, directionDraw, Color.magenta);
        }
        ...
        public CollideResult Move(Vector3 deltaMovement)
        {
            CollideResult collideResult = CollideResult.None;
            ...
            if (deltaMovement.x != 0f)
                MoveHorizontally(ref deltaMovement,ref collideResult);

            // next, check movement in the vertical dir
            if (deltaMovement.y != 0f)
                MoveVertically(ref deltaMovement,ref collideResult);
        ...
        void PrimeRaycastOrigins()
        {
            ...
            modifiedBounds = boxCollider.bounds;
        ...
        private Vector3 directionDraw;
        private Bounds modifiedBounds;
        
        void MoveHorizontally(ref Vector3 deltaMovement,ref CollideResult result)
        {
            ...
            for (var i = 0; i < totalHorizontalRays; i++)
            {
                ...
                rayStart = ray;
                directionDraw = rayDirection * rayDistance;
                ...
                if (_raycastHit && _raycastHit.collider)
                {
                    hitIndex = i;
                    if (collide)
                    {
                    ...
                    result |= CollideResult.Horizontal;
            ...
            int minRaysHit = totalHorizontalRays / 4;
            if (unhitCount != totalHorizontalRays && unhitCount >= totalHorizontalRays - minRaysHit)
            {
                if (hitIndex < totalHorizontalRays / 2)
                {
                    deltaMovement.y += 0.1f;
                }
                else
                {
                    deltaMovement.y -= 0.1f;
        ...
        private void MoveVertically(ref Vector3 deltaMovement,ref CollideResult collideResult)
        {
            ...
            for (var i = 0; i < totalVerticalRays; i++)
            {
                ...
                if (_raycastHit && _raycastHit.collider)
                {
                    ...
                    collideResult |= CollideResult.Vertical;
                    _raycastHitsThisFrame.Add(_raycastHit);
            ...
            int minRaysHit = totalVerticalRays / 4;
            if (unhitCount != totalVerticalRays && unhitCount >= totalVerticalRays - minRaysHit)
            {
                if (hitIndex < totalVerticalRays / 2)
                {
                    deltaMovement.x += 0.1f;
                }
                else
                {
                    deltaMovement.x -= 0.1f;
