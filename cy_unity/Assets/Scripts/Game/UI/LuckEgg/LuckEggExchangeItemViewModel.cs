...
{
    public partial class LuckEggExchangeItemViewModel : ViewModelBase
    {
        ...
        [Preserve]
        public LuckEggExchangeItemViewModel(ExchangeShopCfg cfg)
        {
	        this.cfg = cfg;
	        InitRewardList();
	        InitUI();
        ...
        private async void OnClickBtn()
        {
	        ...
	        if (!UIHelper.CheckRewardIsEnough(cfg.Consumption,true))return;
	        ...
	        var req = new ReqLuckyEggExchange()
	        {
		        Id = cfg.Id,
		        OptionalIndex = Data.GetOptionalSelectIndex(cfg.Id,2)
	        };
	        var result = await GameNetworkManager.Instance.SendAsync<RspLuckyEggExchange>(req);
