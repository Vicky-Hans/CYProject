...
namespace DH.Config 
{
	...
	[DHDataTableObject]
	[MessagePackObject]
	public partial class MainGrindingCfg
	{
		[Key(0)]
		...
		public MainGrindingCfg()
		{
		}

		[SerializationConstructor]
		public MainGrindingCfg(int Id, int NextId, int CustomId, int NowWave, int Time, int GringTime, int Incident, int HpCoefficient, int AtkCoefficient, List<Grinding> Summon)
		{
			this.Id = Id;
			this.NextId = NextId;
	...
	public partial class MainGrindingCfgCollection : CfgCollectionBase<MainGrindingCfg>
	{
		private const string dataPath = "MainGrinding";
		private Dictionary<int, MainGrindingCfg> idMapItems;
		...
		public override void Load()
		{
			...
			var config = DataTableManager.LoadTable<MainGrindingCfg>(dataPath);
			dataItems = config.Data;

			idMapItems = new Dictionary<int, MainGrindingCfg>();
		...
		public override async UniTask LoadAsync()
		{
			...
			var config = await DataTableManager.LoadTableAsync<MainGrindingCfg>(dataPath);
			dataItems = config.Data;
		...
		public MainGrindingCfg GetDataById(int id)
		{
			Load();
