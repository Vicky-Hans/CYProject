	...
	public class FilterByNameAndType : IFilter {
		...
		public bool FilterAsset(AssetReference a) {
			...
			if(keywordMatch && m_filterKeytype != Model.Settings.DEFAULT_FILTER_KEYTYPE) 
			{
                ...
                //Text or Binary特殊处理:Lua.bytes导入时是TextScriptImporter,但TextScriptImporter是internal class,
                //无法直接在CustomFilterTypeUtility.s_defaultAssetFilterGUITypeMap中添加,只能判断字符串
                if (!match && m_filterKeytype.Equals("Text or Binary"))
                {
	                if (incomingType.ToString().Equals("UnityEditor.TextScriptImporter"))
	                {
		                match = true;
	                }
                }
