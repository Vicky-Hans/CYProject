...
namespace LitJson
{
    ...
    public class JsonMapper
    {
        ...
        private static void AddTypeProperties (Type type)
        {
            ...
            var ignore = typeof(DHFramework.DHUtility.Json.JsonIgnoreAttribute);
            ...
            foreach (PropertyInfo p_info in type.GetProperties ()) {
                if (p_info.Name == "Item")
                    continue;
                if(p_info.IsDefined(ignore,false))
                    continue;
            ...
            foreach (FieldInfo f_info in type.GetFields ()) {
                if(f_info.IsDefined(ignore,false))
                    continue;
                PropertyMetadata p_data = new PropertyMetadata ();
                p_data.Info = f_info;
                p_data.IsField = true;
        ...
        private static object ReadValue (Type inst_type, JsonReader reader)
        {
            ...
            if (reader.Token == JsonToken.ArrayStart) {
                ...
                while (true) {
                    ...
                    if (t_data.Properties.ContainsKey (property)) {
                        ...
                        if (converter != null)
                        {
                            keyProperty = converter.ConvertFromString((string)property);
                            t_data.ElementType = dicTypes[1];
                        }

                        ((IDictionary) instance).Add (
                            keyProperty != null ? keyProperty : property, ReadValue (
         ...
         private static object ReadValueBeta (Type inst_type, JsonReader reader)
        {
            reader.Read ();
            ...
            if (reader.Token == JsonToken.ArrayStart) {
                ...
                while (true) {
                    object item = ReadValueBeta (elem_type, reader);
                    if (item == null && reader.Token == JsonToken.ArrayEnd)
                        break;
                ...
                // 无法正确识别的类型，都使用key value形式存储
                if (value_type == typeof(object))
                {
                    value_type = typeof(Dictionary<string, object>);
                }
                ...
                while (true) {
                    ...
                    if (t_data.Properties.ContainsKey (property)) {
                        ...
                        if (prop_data.IsField) {
                            ((FieldInfo) prop_data.Info).SetValue (
                                instance, ReadValueBeta (prop_data.Type, reader));
                        } else {
                            PropertyInfo p_info =
                                (PropertyInfo) prop_data.Info;
                            ...
                            if (p_info.CanWrite)
                                p_info.SetValue (
                                    instance,
                                    ReadValueBeta (prop_data.Type, reader),
                                    null);
                            else
                                ReadValueBeta (prop_data.Type, reader);
                        }
                        ...
                        ((IDictionary) instance).Add (
                            keyProperty != null ? keyProperty : property, ReadValueBeta (
                                t_data.ElementType, reader));
                    }
        ...
        public static T ToObjectBeta<T> (string json)
        {
            ...
            return (T) ReadValueBeta (typeof (T), reader);
        }
