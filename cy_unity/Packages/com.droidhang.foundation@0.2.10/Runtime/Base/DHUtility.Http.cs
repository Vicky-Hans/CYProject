...
namespace DHFramework
{
    ...
    public static partial class DHUtility
    {
        ...
        {
            object SendRequest(HTTPMethods method, string url, Dictionary<string, string> header,string data,
                Action<int,string, string> callback,bool urlEncoded = true);
        }
        ...
        {
            private static IHttpHelper m_httpHelper;
            
            public static Func<string, int, Func<IPAddress[], int, string>,bool> m_DnsResolver;

            public static void SetHttpHelper(IHttpHelper helper)
            {
                m_httpHelper = helper;
                DHLog.Debug("Set http helper");
            }

            public static void SetDnsResolver(Func<string, int, Func<IPAddress[], int, string>,bool> resolver)
            {
                m_DnsResolver = resolver;
                DHLog.Debug("Set dns resolver");
            }
            
            public static object SendRequest(HTTPMethods method, string url,
                Action<int,string, string> callback)
            {
                if (m_httpHelper == null)
                {
                    Debug.LogError("DHUtility.Http did not have valid HttpHelper, please call SetHttpHelper first");
                    return null;
                }
                else
                {
                    return m_httpHelper.SendRequest(method,url,null,null,callback,false);
                }
            }
            
            
            public static object SendRequest(HTTPMethods method, string url, Dictionary<string, string> header,
                Action<int,string, string> callback)
            {
                if (m_httpHelper == null)
                {
                ...
                {
                    return m_httpHelper.SendRequest(method,url,header,null, callback,false);
                }
            }
            
            public static object SendUrlEncodedRequest(HTTPMethods method, string url, Dictionary<string, string> header,string data,
                Action<int,string, string> callback)
            {
                if (m_httpHelper == null)
                {
                ...
                {
                    return m_httpHelper.SendRequest(method,url,header,data,callback);
                }
            }
            
            public static object SendRequest(HTTPMethods method, string url, string headerJson,string data,
                Action<int,string, string> callback)
            {
                if (m_httpHelper == null)
                {
                ...
                else
                {
                    ...
                    {
                        return m_httpHelper.SendRequest(method,url,null,data,callback,urlEncoded:false);
                    }
                    else
                    {
                        var header = DHUtility.Json.ToObject<Dictionary<string, string>>(headerJson);
                        return m_httpHelper.SendRequest(method,url,header,data,callback,urlEncoded:false);
                    }
                }
            }
        }
