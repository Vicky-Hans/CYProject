...
namespace DH.Launch.Network
{
    public class NetworkManager : MonoSingleton<NetworkManager>
    {
        ...
        private void UpdateConnectWrap(ConnectWrap connectWrap)
        {
            ...
            while ((incomingMessage = netClient.ReadMessage(connectWrap.ConnectId)) != null)
            {
        ...
        private void HandleIncomingMessageData(ConnectWrap connectWrap, byte group, byte cmd, byte[] data)
        {
            ...
            //DHLog.Debug($"Application data :  {Encoding.UTF8.GetString(data)}");
        ...
        public void Reconnect(string connectId)
        {
            if (!networkConnectDic.TryGetValue(connectId, out var connectWrap))
            {
                return;
            ...
            if (connectWrap.ServerRefuse) {
                ...
                DHLog.Debug("[NetworkComponent] 服务器拒绝重连,玩家手动重连");   
            }
            else {
                DHLog.Debug("[NetworkComponent] 玩家手动重连");   
        ...
        public ConnectWrapEvent GetConnectWrapEvent(string connectId)
        {
