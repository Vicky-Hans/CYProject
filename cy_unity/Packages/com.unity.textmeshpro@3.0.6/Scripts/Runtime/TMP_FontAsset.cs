...
namespace TMPro
{
    ...
    public class TMP_FontAsset : TMP_Asset
    {
        ...
        */
        ...
        public delegate uint GetGlyphFunc(uint unicode, out FontEngineError error);

        public GetGlyphFunc getGlyphFunc;
        
        public uint GetGlyphIndex(uint unicode,out  FontEngineError error)
        {
            // Load font face.
            error = FontEngine.LoadFontFace(sourceFontFile, m_FaceInfo.pointSize);
        ...
        internal bool TryAddCharacterInternal(uint unicode, out TMP_Character character)
        {
            ...
            uint glyphIndex = getGlyphFunc != null ? getGlyphFunc(unicode,out error) : GetGlyphIndex(unicode, out error);
            if (error != FontEngineError.Success)
            {
                k_TryAddCharacterMarker.End();
                return false;
