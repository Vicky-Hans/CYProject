...
namespace TMPro
{
    ...
    public partial class TextMeshProUGUI
    {
        ...
        protected override void Awake()
        {
            ...
            TMP_Localization.AddLanguageChangedEvent(GetInstanceID(), gameObject.tag, TMP_LocalizationOnLanguageChangedEvent);
        }
        
        private void TMP_LocalizationOnLanguageChangedEvent()
        {
            string tmpTag = gameObject.tag;
            
            var fontAsset = TMP_Localization.GetFont(tmpTag);
            var sharedMaterial = TMP_Localization.GetMaterial(m_sharedMaterial, tmpTag);
            
            if (fontAsset != m_fontAsset && sharedMaterial)
            {
                font = fontAsset;
                fontSharedMaterial = sharedMaterial;
            }
        }
        ...
        protected override void OnDestroy()
        {
            try
            {
                ...
                TMP_Localization.RemoveLanguageChangedEvent(GetInstanceID(), gameObject.tag);
            ...
            {
                Debug.LogError($"[TMP][OnDestroy] exception:{e.Message}\n{e.StackTrace}");
            }
        }
        ...
        protected override void LoadFontAsset()
        {
            ...
            if (m_fontAsset != null)
            {
                ...
                if (fontAsset && fontAsset != m_fontAsset)
                {
                    var sharedMaterial = TMP_Localization.GetMaterial(m_sharedMaterial, tmpTag);
                    if (sharedMaterial)
                    {
