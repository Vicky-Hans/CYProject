...
namespace DHHybridCLR.Editor.BuildPipeline
{
    ...
    {
        [MenuItem("HybridCLRxLua/Tools/GlobalMetadata/合并代码到单独文件", false, 1002)]
        private static void CombineLzma()
        {
            ...
            var saveName = EditorUtility.SaveFilePanel("保存为...",
                "选择要保存的文件目录", "HyclrXluaCppModule", "bytes");
            if (string.IsNullOrEmpty(saveName))
            {
                UnityEngine.Debug.LogError("你没有选择保存的位置..., 保存失败");
            ...
            UnityEngine.Debug.Log($"合并目录{pathName} 到 {saveName}成功!!!");
        }
        ...
        private static bool ExtractApiCodeToPath(string fileName, string outPath)
        {
            ...
            {
                UnityEngine.Debug.LogError($"目录{outPath}不存在!!!");
                return false;
            }

            if (!File.Exists(fileName))
            {
                UnityEngine.Debug.LogError($"文件{fileName}不存在!!!");
                return false;
            }
        ...
        [MenuItem("HybridCLRxLua/Tools/GlobalMetadata/从文件里拆解代码到指定目录", false, 1003)]
